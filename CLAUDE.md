...
- **`/jobs`** — read-only snapshot of in-flight work in the current chat: active runs (engine, elapsed minutes, pid + liveness, triggering cron/webhook) sourced from `RunningTask.engine`/`started_at`/`pid` in `running_tasks`, plus counts of jobs queued behind a session lock and pending `/at` delays. No admin gate, matching `/ping`. See `telegram/jobs.py`
- **Antigravity heartbeat & liveness** — for envelope-only engines (`streams_progress = False`, e.g. Antigravity) that emit no `ActionEvent`s, the stall monitor now periodically refreshes the elapsed-time heartbeat and surfaces a `meta["liveness"]` line (`process alive`/`exited`, `CPU active`/`idle`) so a run in progress doesn't look stuck; the stall threshold itself is derived from the runner's `expected_silence_budget_s()` when it provides one, instead of the generic fallback. See `_update_silent_engine_liveness()` in `runner_bridge.py` and `AntigravityRunner.expected_silence_budget_s()` in `runners/antigravity.py`
